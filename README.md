...
Three Rust smart contracts that run neural network inference **entirely on-chain**, plus a Cloudflare Worker that serves the frontends through custom domains.
...
All three use **integer quantization** with per-layer scales so inference is deterministic and cheap inside the WASM sandbox.
