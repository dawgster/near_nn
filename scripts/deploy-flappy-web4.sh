...
usage() {
  cat <<'EOF'
Usage:
  scripts/deploy-flappy-web4.sh <contract-account-id> [testnet|mainnet]

Examples:
  scripts/deploy-flappy-web4.sh my-flappy-demo.testnet
  SIGNER_ACCOUNT_ID=my-flappy-demo.testnet scripts/deploy-flappy-web4.sh my-flappy-demo.testnet testnet
...
Environment variables:
  ...
  RUN_SMOKE_TEST      Set to 0 to skip the final game_config view call.
EOF
}
...
contract_dir="${repo_root}/flappy_bird_contract"
...
wasm_path="${build_target_dir}/near/near_flappy_bird_demo.wasm"
...
echo "Building Flappy Web4 contract for ${contract_id} on ${network}..."
(
  cd "${contract_dir}"
...
if [[ "${run_smoke_test}" != "0" ]]; then
  echo "Running smoke test: game_config"
  near contract call-function as-read-only "${contract_id}" game_config json-args '{}' \
    network-config "${network}" \
    now
fi
