...
struct Prediction {
    hidden_layer: Vec<i32>,
    output_layer: Vec<i32>,
    predicted_class: bool,
}

#[tokio::test]
async fn runs_xor_model_in_near_sandbox() -> Result<()> {
    ...
    let prediction: Data<Prediction> = Contract(contract_id.clone())
        .call_function("predict", json!({ "inputs": [1000, 0] }))
        .read_only()
        .fetch_from(&network)
        .await?;

    assert!(prediction.data.predicted_class);
    assert_eq!(prediction.data.hidden_layer, vec![1000, 0]);
    assert_eq!(prediction.data.output_layer, vec![1000]);

    let truth_table: Data<Vec<Prediction>> = Contract(contract_id)
        .call_function("xor_truth_table", ())
        .read_only()
        .fetch_from(&network)
        .await?;
    let classes: Vec<bool> = truth_table
        .data
        .iter()
        .map(|row| row.predicted_class)
        .collect();
...
fn wasm_artifact_path() -> PathBuf {
    ...
    let candidates = [
        near_target_dir
            .join("near_nn_demo")
            .join("near_nn_demo.wasm"),
        near_target_dir.join("near_nn_demo.wasm"),
        target_dir
